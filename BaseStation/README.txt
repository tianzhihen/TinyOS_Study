...
When using a CC2420/RFA1 radio, several default preprocessor configurations
are defined in the Makefile:
  * CC2420_NO_ACKNOWLEDGEMENTS RFA1_NO_ACKNOWLEDGEMENTS
    - Prevents the base station from falsly acknowledging packets
  * CC2420_NO_ADDRESS_RECOGNITION RFA1_NO_ADDRESS_RECOGNITION
...
Other combinations can be defined to meet your application's needs:
  * CC2420_NO_ADDRESS_RECOGNITION or RFA1_NO_ADDRESS_RECOGNITION only
    - Sniff all packets, but acknowledge packets only if they
      are sent to the base station's address (RFA1: if ack required)
